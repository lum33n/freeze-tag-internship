...
\clem{Dans cette section, nous montrerons le résultat suivant, obtenu pendant
...
On prouvera:
\todo[inline]{introduire un peu le lemme, dire que le théorème principal en découle directement.\\
  Introduire $\gamma$ et $P$ avant le lemme. Combien de points y a t il dans $P$ ?
  Si $|P|$ est constitué de 5 points  pourquoi $\gamma$ serait un chemin ?\\}
...
\todo{Il faut éviter autant que possible les formules mathématiques dans le
  raisonnement logique. Le symbole $\Rightarrow$ a un sens logique, on préfèrera
écrire ``Preuve de \ref{alpha5borne} en supposant Lemme \ref{chord}''. De même
éviter d'utiliser $\forall$ et $\exists$, $\le$ dans le texte.}
...
\todo{C'est évident à partir du contexte mais il faut dire qui sont les $\theta_k$.}
...
\clem{Soit $\alpha > 0$ et $P$ ...} Tout d'abord on tournera notre figure en sélectionnant un point tel que son axe coupe le cercle en deux demi-cercles contenant chacun 2 points. Un tel point existe toujours.

\includegraphics[scale=0.5]{initial_position}

On sépare ensuite le cercle en différents cônes d'angle $\alpha$ nommés $A$,
$B$, $C$, et $D$ \clem{voir Figure ...}. C'est en effet ce $\alpha$ qui apparaîtra dans la formule finale. On notera $|X|$ le nombre de point dans le cône $X$ et nous ferons des disjonctions de cas sur ces derniers.

Soit $P$ un ensemble de point, on tourne le cercle comme prévu. Pour l'exemple,
...
prévue.\clem{Pas besoin de reparler de la rotation et de $P$. La dernière phrase
  est assez vague.}

\subsubsection*{1er cas}\clem{Il existe un cône $X \in \{A,B,C,D\}$ tel que $|X|
  \ge 2$} $\exists X, |X| \geq 2$
...
\includegraphics[scale=0.5]{2eme_cas}
...
\includegraphics[scale=0.45]{3eme_cas1}
...
\includegraphics[scale=0.45]{3eme_cas2}
...
\includegraphics[scale=0.45]{3eme_cas3}
...
\includegraphics[scale=0.5]{4eme_cas}
